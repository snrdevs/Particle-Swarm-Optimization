...
no_design_variable = 3;                             % # of Design Variables
...
dt = 1;                                             % Time Step
%% Tension Spring Design Problem Definition  

PCONST = 1000000;                                      % Penalty Function Constant

objective_function = @(x) (x(3)+2)*x(2)*(x(1)^2);      % Objective Function f(x)

g1 = @(x) 1-((x(2)^3 * x(3))/(7178*(x(1)^4)));
g2 = @(x) ((4*x(2)^2 - x(1)*x(2))/(12566 *(x(2)* x(1)^3)))  + (1/5108*x(1)^2) - 1;  
g3 = @(x) 1 - ((140.45*x(1))/(x(2)^2*x(3)));
g4 = @(x) ((x(2) + x(1)) / 1.5) - 1;

penalty_function = @(x) objective_function(x) + PCONST*(max(0,g1(x))^2+max(0,g2(x))^2+max(0,g3(x))^2+max(0,g4(x))^2);% Penalty Function
